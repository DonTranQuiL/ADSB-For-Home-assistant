...
from .const import (
    ...
    CONF_TRACKED_LIST, CONF_ADD_TRACK, CONF_REMOVE_TRACK, CONF_CLEAR_TRACK,
    MODE_SINGLE, MODE_ZONE
)
...
class AirplanesLiveConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_zone(self, user_input=None):
        if user_input is not None:
            return self.async_create_entry(title="Zone Tracking", data={CONF_TRACKING_MODE: MODE_ZONE, **user_input})
    ...
    async def async_step_single(self, user_input=None):
        if user_input is not None:
            return self.async_create_entry(title="Target Tracker", data={CONF_TRACKING_MODE: MODE_SINGLE, **user_input})
        return self.async_show_form(step_id="single", data_schema=vol.Schema({
            vol.Required(CONF_IDENTIFIER_TYPE, default="callsign"): vol.In({"callsign": "Callsign", "reg": "Registration", "hex": "ICAO Hex"}),
...
class AirplanesLiveOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        current_radius = options.get(CONF_RADIUS, self._config_entry.data.get(CONF_RADIUS, 5000))
