...
@pytest.fixture
def mock_api():
    """Mock the API module layout out explicitly."""
...
def test_aircraft_classification_categories(hass):
...
async def test_coordinator_zone_analytics(hass: HomeAssistant, mock_api):
    ...
    mock_api.get_aircraft_in_zone.return_value = [
        ...
        {"hex": "D4E5F6", "lat": 59.000, "lon": 9.000, "desc": "Boeing", "flight": "FAR_AWAY"} # Out of radius filter
    ]
    ...
    assert result["entered"] == 0 # No reference map on run one

    # 2. Second Run: Map changes to verify "Entered" and "Exited" telemetry tracking
    mock_api.get_aircraft_in_zone.return_value = [
        {"hex": "A1B2C3", "lat": 52.001, "lon": 5.001, "desc": "Military Drone", "flight": "MIL1"}, # Kept
        {"hex": "NEW777", "lat": 52.002, "lon": 5.002, "desc": "Rotor-craft", "flight": "HELI"}     # Entered
    ]
